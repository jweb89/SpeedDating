...
d_interests_correlate,d_expected_happy_with_sd_people,d_expected_num_interested_in_me,d_expected_num_matches,d_guess_prob_liked,d_like
))
...
```{r}
# Convert b'1' to 1
speed_dating = speed_dating %>% mutate_at(vars(decision, decision_o, match, gender, race, has_null, race_o, samerace, field, ), function(x) gsub("^b'|'$", "", x))
...
mdl = glm(match ~ criteria_o_percent + criteria_percent, test, family = 'binomial')
...
pred1[prob1>0.6]=1 ## if predicted probability is more than 60%, we mark that as a survived.
