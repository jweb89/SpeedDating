...
Speed dating is a social event that has become increasingly popular over the years. It is a structured way for singles to meet and connect with many potential partners in a short amount of time. During a speed dating event, participants typically rotate through a series of short interactions, spending a few minutes chatting with each person before moving on to the next. This format allows singles to quickly assess their compatibility with a variety of individuals, and find someone with whom they share a mutual attraction. The concept of speed dating has been adapted to various age groups, interests, and orientations, making it a versatile and exciting way to meet new people.
...
The data set contains information gathered during speed dating events from 2002 to 2004. Each interaction lasted four minutes, and participants took part in a total of twenty dates over the course of the event. Following the four minutes, participants and their partners recorded whether or not they would like to meet again. If both individuals recorded a yes, they were considered a match. A large variety of variables about demographic information, interests, preferences, and prior beliefs are included in the data set, each of which will be described in the following paragraphs. Each observation represents one four minute date, with information about both the participant and partner.
The `gender` variable simply tells whether the participant was male or female. The study did not include any gender other than male and female. The `age` and `age_o` variables give the age of the participant and other, respectively. The `race` variable denotes the race of the participant, while the `race_o` variable records the race of the partner. A total of six unique options occurred, including Asian/Pacific-Islander/Asian-American, European/Caucasian-American, Black/African-American, Latino/Hispanic-American, as well as other and “?”. The `samerace` variable indicates whether or not the participant and partner belonged to the same race in a binary format. The `importance_same_race` variable displays how important the participant feels that the other individual is of the same race. The `importance_same_religion` variable works in the same manner, indicating how important the participant feels it is that their partner is of the same religion. The `field` variable records the field of work/study which the participant belongs to, with 260 unique options.

A variety of variables in the data set refer to traits, including attractiveness, sincerity, intelligence, funniness, ambition, and shared interests. In order to prevent listing these individually, we will provide the format for the variables using attractiveness, and note that the other traits can be substituted respectively. The `pref_o_attractive` variable provides a value for how important attractiveness is to the partner. As mentioned, variables in the same “pref_o_trait” format exist for all of the six aforementioned traits. The `attractive_o` variable gives a rating by the partner about the participant’s attractiveness. The `attractive_important` variable provides a value for how important attractiveness is to the participant. The `attractive_partner` variable gives a rating by the participant about the partner’s attractiveness. Finally, the `attractive` variable gives a self-perceived rating of the participant. It is important to note that the values for the trait importance variables exist from 0-100, with the total across all six traits summing to 100. These can be thought of as percentages.

Seventeen different variables exist which record how important a specific interest is to the participant. These interests include sports, sports on tv, exercise, dining, museums, art, hiking, gaming, clubbing, reading, tv, theater, movies, concerts, music, shopping, and yoga. The `interests_correlate` variable provides a correlation between interests of the participant and partner, although interest ratings for the partner are not included in the data set.

The `expected_happy_with_sd_people` provides a score for how happy the participants expect to be with the people they meet during the event. The `expected_num_interested` variable indicates how many people the participant feels will be interested in meeting them again out of twenty. The `expected_num_matches` variable works similarly, denoting the number of expected matches out of twenty. The `like` variable gives a rating for how much the participant liked their partner, while the `guess_prob_liked` variable provides a value for how likely the participant feels it is that their partner liked them.

The `met` variable records whether or not the participant and partner have met before, in a binary format. The `decision` variable provides the decision by the participant whether or not to meet again, and the `decision_o` variable provides the decision by the partner whether or not to meet again, both in a binary format. The `match` variable indicates whether or not the individuals would both like to meet again, based on the decision variables, in a binary format.

We created two variables `criteria_percent` and `criteria_o_percent` which were derived by multiplying the importance ratings(adding up to 100) and the ratings of those traits(0 - 10) and dividing by 1000. This yielded a number between 0 and 1 that represented the weighted percentage of the importance factor the person and than partner gave each other.

The `male_with_desireable_mate` and `female_with_desireable_mate` are datasets filtered such that they contain dating encounters where the counterpart was an 8 or higher in their most desired category.  The 'model_whats_important_male' and 'model_whats_important_female' are the logistic models that attempt to explain the probability of a person desiring a date given their score on a mate's most desired trait.

## Exploratory Data Analysis: Descriptive Statistics and Visualizations

```{r results = "markup"}
df <- data.frame() 
num_vars <- sapply(speed_dating, is.numeric) 
for(n in names(speed_dating)[num_vars]) {
  output <- favstats(speed_dating[[n]])
  df <- rbind(df, output)
}

variable <- names(speed_dating)[num_vars]

summarytable <- cbind(variable, df)
knitr::kable(summarytable, row.names = FALSE)
```

The above table shows summary statistics for all numeric variables in the data set, other than those initially removed.
...
Across the entire data set, approximately 16.47% of dates resulted in a match. While men are likely to want to date their partner nearly 50% of the time, women are much less so at about 37%.
...
The above table shows the average percentage of importance for six traits, colored by men and women. This chart shows that men tend to weigh attractiveness as the most important trait while women are more evenly spread out among the importance of the traits. This idea surrounding the most important trait will be explored in our first research question.
...
Due to the vast number of variables included in our data set, we have provided a filtered correlation matrix which only displays variables with correlations greater than .7. As can be seen from the display, the two variables with the strongest correlation are art and museums. This correlation is intuitive in nature, since museums often display art, and art is often displayed in museums. Therefore, participants who rated higher interest in museums, would be expected to also rate higher interest in art. Two other interestingly high correlations are those between `criteria_percent` and `funny_partner`, as well as `criteria_o_percent` and `funny_o`. Based on these, it can be stated that as participant's rating of their partner as funny increased, the aggregate percent of their criteria fulfilled by their partner also increased. This association can also be seen from the perspective of the partner.
...
In examining this question, we started by looking at the data set filtered to a data set consisting of men only, and another data set consisting of women only.  In order to look into this theory further, we created two new columns in each data set.  The first column was calculated by selecting the trait or characteristic that the person assigned the importance.  The next column took the rating that the person assigned as the most important, and assigned the rating (0-100) as a measure of relative importance.  
As mention earlier, mens' preferences most often point to the importance of attractiveness, with intelligence and sincerity occurring much less frequently.  On the other hand, women tend to have a more balanced set of desired traits.  The highest scores for womens' desired traits are sincerity, attractiveness, and intelligence.
...
The result for the women seems to be a bit better, accurately predicting 77% of choices to date or not to date.
...
```
Can we accurately predict what a man will do when his counterpart ’s most desired characteristic is scored by them as 8 or higher? In this condition, they are likely to request a date over 72% of the time.

What if we filter women where their MOST DESIRED characteristic is greater than 8?
...
If a woman’s most desired trait is 8 or higher, she is only 57% likely to request a date.  This seems to affirm that more than one trait is important to women.
...
### Following variable selection, creation of training and test data, building a logistic model, and removing multicollinearity
...
for(param in params){
  ...
  plot = ggplot(data, aes(x= .data[[param]]))+
    geom_smooth( aes(y=.fitted))
  plots[[i]] = plot
  i = i + 1
}

# Split the plots in 2 so it shows better
my_list_split <- split(plots, f = rep(1:2, length.out = length(plots)))
first_half <- my_list_split[[1]]
second_half <- my_list_split[[2]]
ggarrange(plotlist = first_half)
ggarrange(plotlist = second_half)
...
```{r results = "markup"}
knitr::kable(APIAAmodelAIC$coefficients)
```

The above table displays the final model coefficients after choosing the lowest Akaike Information Criterion.

```{r results = "markup"}
knitr::kable(vif(APIAAmodelAIC))
```

The above table shows the Variance Inflation Factor ratios of the included coefficients.
...
```{r results = "markup"}
knitr::kable(ECAmodelAIC$coefficients)
```

The above table displays the final model coefficients after choosing the lowest Akaike Information Criterion.

```{r results = "markup"}
knitr::kable(vif(ECAmodelAIC))
```
...
```{r results = "markup"}
knitr::kable(BAAmodelAIC$coefficients)
```

The above table displays the final model coefficients after choosing the lowest Akaike Information Criterion.

```{r results = "markup"}
knitr::kable(vif(BAAmodelAIC))
```
...
```{r results = "markup"}
knitr::kable(LHAmodelAIC$coefficients)
```

The above table displays the final model coefficients after choosing the lowest Akaike Information Criterion.

```{r results = "markup"}
knitr::kable(vif(LHAmodelAIC))
```
...
## Race Models Discussion and Comparison

One of the most unexpected findings of the models we created by race is that increasing values in trait importance, across attractiveness, sincerity, intelligence, funniness, and ambition, actually results in reduced odds of the participant's decision equaling yes. Prior to this discovery, we had theorized that individuals with certain important traits may be more or less likely to choose matches, and that this could differ across races. While some of the trait importance variables did not make it into all of the final models, each and every one is negatively associated with odds of the participant's decision to choose yes. After spending some time thinking this over, we believe that it makes sense. Individuals who are more picky, and therefore hold traits to be more important prior to even meeting a potential partner, are less likely to want to match. The more heavily weighted a certain trait is, the greater chance the participant may be disappointed by their partner.

Another interesting finding is that `gender` is only included in both the APIAA and ECA models. In both cases, when the participant is male, their odds of deciding yes to a match are greatly increased. This may indicate that males who belong to the APIAA and ECA groups are more desperate to find a partner, or that males from the BAA and LHA groups are more thoughtful in doing so.

A second surprising result of our models is that with any increase in `expected_happy_with_sd_people`, a participant is less likely to decide yes across all four races. This was not an expected result, as we assumed that someone coming into the speed dating event with a more optimistic mindset would surely be more likely to desire matches. Across the `importance_same_race` and `importance_same_religion` variables, all final models resulted in a negative association when included, other than the BAA model. Interestingly enough, for these participants, increases in the importance of their partner being the same religion resulted in greater odds of deciding yes. While we are unsure of the reasoning which may have led to this difference, we would argue that the general trend of decreasing odds with increased feelings of importance is a result of more particular participants, who are less open-minded and may hold many pre-conceived notions prior to a speed date.

A few other points to note are in regards to the `expected_num_matches`, `interests_correlate`, and `criteria_percent` variables. In all four of the race models, increases in the expected number of matches resulted in increased odds of deciding yes. Our guess is that the reasoning for this is simple, such that individuals who expect more matches are more likely to agree to more matches and avoid disappointment. The `interests_correlate` variable only appeared in the ECA model, which indicates that how well the interests of a participant and their partner match may not be of as much importance as is often thought in today's society. In all four of the models, the `criteria_percent` variable is positively associated with the odds of deciding yes, which is to be expected. A partner who "fits the bill" of a participant's desires is surely more likely to be sought out for a match.

Before moving into the conclusion of our project, we would like to mention a couple of important things. First, the variables used to create the models by race were by no means exhaustive of those in the data set, and therefore are likely to be missing lots of important information. It is unreasonable to think that a model which includes information almost solely related to one participant would be entirely accurate in making predictions about the outcome of a two-person scenario. Second, even with the model outcomes only considering the decision of the participant, they are still relatively inaccurate, as displayed by the confusion matrices. For these reasons, we do not feel that the race model results are able to be confidently generalized to speed dating events, and believe that further research into the differences in outcomes across individuals of different ethnic backgrounds would be interesting.
...
In our second question, we also had somewhat disappointing results. The accuracy was good, but as we mentioned previously it was not very accurate at predicting matches based on the data available despite using the as many parameters as would improve the model. This further speaks to the idea that human interaction is extremely complex and there are surely many aspects of these 4 minutes speed dates that are not captured in the data set.

Our final research question sought out differences in speed dating preferences and outcomes based on race. Our exploratory data analysis yielded a few interesting results in this regard, such as a dominant importance towards attractiveness across all races. Additionally, we saw that participants of more collectivist cultures seemed to value shared interests and sincerity slightly more. The logistic regression models built for individual races also revealed some interesting results, as discussed in the prior section. However, we feel that a more complete set of variables should be included in future research to acquire a more accurate assessment.
...
3. https://www.kaggle.com/datasets/ulrikthygepedersen/speed-dating

# Contributions
We are all satisfied with group member contributions

Mike: Identified several research papers and datasets related to weather (not used), wrote an initial draft of the paper, wrote the initial draft of the presentation, research question #1, located references, some dataset manipulation for the purposes of answering research question.

Jared: Identified several datasets that were sports and betting-related.  Drove the effort to moving the paper into R for a more seamless environment.  Answered research question #3.  Provided much of the descriptive statistics section.

Josh: Found the dataset that we moved forward with to create this paper.  Provided much of the initial programming to get the dataset to a usable state along with transformations to answer research question #2.  Created a common code environment on R-Studio Cloud that we used at the start, then migrated us to a Github repository for shared code.  
